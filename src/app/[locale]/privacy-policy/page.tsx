...
export default function PrivacyPolicyPage() {
	...
	return (
		<AppLayout>
			<div className={'px-[var(--container-padding)] py-6 text-[#444444]'}>
				<h3 className={'pb-6 text-xl text-center font-bold'}>{t('privacy_policy_header')}</h3>
				<div className={'pb-6'}>
					<span className={'font-light italic'}>{t('privacy_policy_last_updated')}</span>
				</div>
				<p className={'pb-9'}>{t('privacy_policy_intro')}</p>
				<div className={'pb-9'}>
					<h4 className={'font-bold pb-4'}>{t('personal_information_we_collect')}</h4>
					<p className={'pb-4'}>{t('personal_information_we_collect_description_1')}</p>
					<p dangerouslySetInnerHTML={{
						__html: t('personal_information_we_collect_description_2')
					}} />
				</div>
				<div className={'pb-9'}>
					<h4 className={'font-bold pb-4'}>{t('how_we_use_your_information')}</h4>
					<p dangerouslySetInnerHTML={{
