...
export function ItemsDrawer() {
  ...
  return (
    ...
    >
      <div
        className={
          "px-4 md:px-9 py-4 md:py-6 flex flex-col w-full md:w-[350px] h-full bg-[#FCF7F1] gap-4 md:gap-9"
        }
      >
        <div className={"flex justify-between gap-4 md:gap-9 py-3 md:pt-13"}>
          <h4 className={" font-medium text-sm md:text-base"}>{t("items")}</h4>

          <h4
            className={
              "cursor-pointer font-normal text-xs md:text-sm text-[#ADADAD] hover:text-[var(--text-color)] transition-all touch-manipulation"
            }
            onClick={() => {
        ...
        <div className={"grow overflow-y-auto max-h-[200px] md:max-h-none"}>
          {flattened.map((element) => {
            return (
              <div
                key={`${element.product.product_id}-${element.option.size}`}
        ...
        <div
          className={
            "flex justify-between gap-4 md:gap-9 py-3 md:py-9 border-t border-[#E5E5E5]"
          }
        >
          <h4 className={" font-medium text-sm md:text-base"}>{t("total")}</h4>

          <h4 className={"font-bold text-sm md:text-base"}>{`${formatNumber(
            totalPrice
          )}${EUR_SYMBOL}`}</h4>
        </div>
      </div>
